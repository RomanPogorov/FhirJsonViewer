...
export function JsonEditor({ data, onUpdateJson, focusPath }: JsonEditorProps) {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const pattern = new RegExp(
          `"${escapeRegExp(lastPropertyName)}"\\s*:`,
          "g"
        );
        ...
        if (match) {
          ...
          const isCorrectPath = verifyPathContext(
            lines,
            currentLineIndex,
            pathSegments
          );
          ...
          if (isCorrectPath) {
            console.log(
              "Найдена соответствующая строка для свойства:",
              lastPropertyName,
              "на строке",
              currentLineIndex + 1
            );
      ...
      console.log(
        "Не найдено точное совпадение, ищем любое вхождение ключа:",
        lastPropertyName
      );
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
  ...
  const parseJsonPath = (
    path: string
  ): { key: string; isArray: boolean; index?: number }[] | null => {
    try {
        ...
        if (focusPath) {
          ...
          setTimeout(() => {
            ...
            if (positionData) {
              ...
              setTimeout(() => {
                console.log(
                  "Устанавливаем позицию при монтировании:",
                  positionData
                );
                try {
                  ...
                  console.error(
                    "Ошибка при установке позиции в handleEditorDidMount:",
                    err
                  );
                }
              ...
              console.log(
                "Не удалось найти позицию при монтировании для пути:",
                focusPath
              );
            }
          }, 100);
        ...
        if (focusPath) {
          ...
          setTimeout(() => {
            ...
            if (position) {
              ...
              setTimeout(() => {
                console.log(
                  "Устанавливаем позицию в редакторе из useEffect:",
                  position
                );
                try {
              ...
              console.log(
                "Не удалось найти позицию для пути в useEffect:",
                focusPath
              );
            }
          }, 100);
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-4">
        <div className="flex justify-between items-center">
          ...
          <div className="space-x-2">
            <Button variant="outline" onClick={handleFormatJson} size="sm">
              Format JSON
            </Button>
